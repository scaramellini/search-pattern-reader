...
public class old_MasterMultidetailPattern extends GenericPattern {
    ...
    public List<NavigationFlow> matches(List<NavigationFlow> flows, NavigationFlow current,
        ...
        if (current.getFromElement().equals("List") && current.getToElement().equals("Details")) {
            flows.stream()
                    .filter(flow -> (flow.getFromElement().equals("Details")
                            && flow.getToElement().equals("Details")
                            && flow.getFromId().equals(current.getToId())))
                    .findAny()
                    .ifPresent(flow -> {
